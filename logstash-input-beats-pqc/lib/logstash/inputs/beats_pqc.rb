...
class LogStash::Inputs::BeatsPqc < LogStash::Inputs::Base
  ...
  config :pqc_hybrid_group, :validate => [REQUIRED_GROUP], :default => REQUIRED_GROUP
  ...
  def validate_config!
    ...
    if @pqc_enabled && @pqc_require
      configuration_error("pqc_hybrid_group must be #{REQUIRED_GROUP} when pqc_enabled and pqc_require are true") unless @pqc_hybrid_group == REQUIRED_GROUP
      configuration_error("pqc_allow_fallback must be false when pqc_require is true") if @pqc_allow_fallback
    end
